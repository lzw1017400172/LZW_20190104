...
import com.lzw.core.support.excel.Excel2003Reader;
import com.lzw.core.support.excel.Excel2007Reader;
import com.lzw.core.support.excel.ExcelHead2007Reader;
import com.lzw.core.support.excel.IRowReader;
...
public final class ExcelReaderUtil {
	 ...
	 * @throws Exception
	 */
	public static void readExcelhead(IRowReader reader, String fileName, InputStream inputStream) throws Exception {
		// 处理excel2007文件
		if (fileName.endsWith(EXCEL07_EXTENSION)) {
			ExcelHead2007Reader excel07 = new ExcelHead2007Reader(reader);
			excel07.process(inputStream);
		} else {
			throw new Exception("不是2007excel格式");
		}
	}
	 ...
	 * 获取Excel第一行
	 */
	public static final List<String[]> excelHead(String filePath) throws Exception {
		List<String[]> resultList = null;
		InputStream is = null;
		try {
			is = new FileInputStream(filePath);
			resultList = excelHead(filePath, is);
		} catch (Exception e) {
			throw e;
		} finally {
			if (is != null) {
	 ...
	 * 获取Excel第一行
	 */
	public static final List<String[]> excelHead(String fileName, InputStream is)
			throws Exception {
		final ArrayList<String[]> resultList = new ArrayList<String[]>();
		try {
			readExcelhead(new IRowReader() {
				public void getRows(int sheetIndex, int curRow, List<String> rowlist){
					resultList.add(rowlist.toArray(new String[] {}));
				}
			}, fileName, is);
		} catch (Exception e) {
			if(e.getMessage().equals("first")){
				return resultList;
			}
			logger.error("Read excel error.", e);
			throw e;
