...
public class CheckCodeUtil {
	...
	public static BufferedImage getCheckCode(){
		...
		if(sessionId != null){
			try {
				CacheUtil.getCache().set(Constants.VERIFICATION_CODE + sessionId.toString(),capstr,PropertiesUtil.getInt("checkCode.expiration"));
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
	...
	public static Boolean validateCheckCode(String code){
		...
		Serializable sessionId = SecurityUtils.getSubject().getSession().getId();
		if(sessionId != null){
			String key = Constants.VERIFICATION_CODE + sessionId.toString();
			try {
