...
public final class WebUtil {
    ...
    /** 保存当前用户类型*/
    public static final void saveCurrentUserType(Object userType) {
        setSession(Constants.CURRENT_USER_TYPE, userType);
    }

    /** 保存当前用户类型 */
    public static final void saveCurrentUserType(HttpServletRequest request, Object userType) {
        setSession(request, Constants.CURRENT_USER_TYPE, userType);
    }
    ...
    public static final Long getCurrentUser() {
        ...
        if (null != currentUser) {
            try {
                ...
                if (null != session) {
                    return Long.valueOf(Objects.toString(session.getAttribute(Constants.CURRENT_USER),"-1"));
                }
    ...
    /** 获取当前用户类型 */
    public static final int getCurrentUserType() {
        Subject currentUser = SecurityUtils.getSubject();
        if (null != currentUser) {
            try {
                Session session = currentUser.getSession();
                if (null != session) {
                    return (int)session.getAttribute(Constants.CURRENT_USER_TYPE);
                }
            } catch (InvalidSessionException e) {
                logger.error(e);
            }
    ...
    /** 获取当前用户类型 */
    public static final int getCurrentUserType(HttpServletRequest request) {
        try {
            HttpSession session = request.getSession();
            if (null != session) { 
                return (int)session.getAttribute(Constants.CURRENT_USER_TYPE);
    ...
    /**
     * 将一些数据放到ShiroSession中,以便于其它地方使用，会放到redis中
     * 
