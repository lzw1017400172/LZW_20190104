package com.lzw.core.support.cache;
...
public class RedissonHelper implements CacheManager, ApplicationContextAware {
	...
	public final int getSetKeyOne(final String key) {
		RBucket<Object> temp = getRedisBucket(key);
		Object object = temp.get();
		if(object!=null) {
			return (int)object;
		}else {
			return 0;
	...
	public final void setForEver(final String key, final Serializable value) {
		RBucket<Object> temp = getRedisBucket(key);
		temp.set(value);
	}
	...
	public final boolean clear(final String key) {
		RBucket<Object> rBucket = getRedisBucket(key);
		return rBucket.clearExpire();
	}

	public final Object getSet(final String key, final Serializable value) {
		RBucket<Object> rBucket = getRedisBucket(key);
		Object a=rBucket.getAndSet(value);
		return a;
	}
	...
	public void getSetKeyAll(String pattern) {
		...
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			String key = iterator.next();
			int num =(int) getRedisBucket(key).get();
			num++;
			RBucket<Object> temp = getRedisBucket(key);
			temp.set(num);
		}
	}
	public void setZero(String pattern,String userId) {
		RBucket<Object> temp = getRedisBucket(pattern+userId);
		temp.set(0);
		Iterable<String> keys = getRedis().getKeys().getKeysByPattern(pattern+"*"+userId);
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			String key = iterator.next();
			RBucket<Object> temps = getRedisBucket(key);
			temps.set(0);
		}
	}
