...
public class EventInterceptor extends BaseInterceptor {
	...
	protected BaseProvider cmsProvider;
	...
	public void afterCompletion(final HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		if (!path.contains("/read/") && !path.contains("/unauthorized") && !path.contains("/forbidden") && !path.contains("/modeler.html") && !path.contains("/editor-app")) {
